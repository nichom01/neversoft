...
**Key behaviours:**
- Consumes from `declarations.created` Kafka topic (consumer group: `validate-svc`)
...
**Key behaviours:**
- Consumes from `validations.completed` topic (consumer group: `risk-svc`)
...
- `declare-svc`, `validate-svc`, `risk-svc`, `audit-svc` — the four Quarkus native services
