...
- [x] T002 Implement `.github/workflows/detect-changes.yml` as a `workflow_call` reusable workflow with: inputs `modules` (string, default `"all"`) and `base_ref` (string, optional); hardcoded known-module list `[declare-svc, audit-svc, validate-svc, risk-svc, it-tests, infra]`; git diff logic (`git diff --name-only ${{ inputs.base_ref }}...HEAD`) to filter changed paths by module prefix; full-fallback logic (set `detection_source: full-fallback` and return full list if git diff exits non-zero); outputs `affected_modules` (JSON array string), `run_all` (string `"true"/"false"`), `run_it_tests` (string `"true"/"false"`), `detection_source` (string `detected|manual|full-fallback`); `run_it_tests` set to `"true"` when any of the four services, `infra`, or `it-tests` appear in affected modules; when `inputs.modules` is provided and not `"all"`, return it directly as the affected list with `detection_source: manual`
...
**Independent Test**: Open a PR modifying a single file in `declare-svc/`. Confirm only `declare-svc` appears in build and unit-test matrix jobs. Confirm integration tests run. Confirm all other services report as skipped.
...
**Independent Test**: Navigate to Actions → On-Demand Pipeline → Run workflow. Set `modules: audit-svc`, `ref: main`. Confirm only `audit-svc` runs in build/unit-test matrices. Confirm integration tests run. Confirm job summary is posted.
...
- [x] T009 [P] [US3] Verify independent callability: author a temporary workflow `.github/workflows/scratch-chain-test.yml` that calls only `./.github/workflows/build-module.yml` with `module: declare-svc` and no prior detect step; trigger it manually; confirm it builds successfully; then delete the scratch file
...
- [ ] T010 [P] Validate Quickstart Scenario 1 — open a PR modifying one file in `declare-svc/` and confirm: `detect` outputs `["declare-svc"]`, only one matrix job fires for build and unit-test, integration tests run, other services are absent from all matrix jobs, PR Checks tab shows green
...
- [ ] T012 [P] Validate Quickstart Scenario 6 — introduce a deliberate compilation error in `declare-svc` on a test branch, open a PR touching both `declare-svc` and `audit-svc`, and confirm: `declare-svc` build job fails, `audit-svc` build job still runs, PR is marked failed with per-module status visible in the summary
